import { CheckCircle2, Clipboard, MapPinned, Navigation, ShieldAlert, Sparkles, Timer, Wrench } from "lucide-react";
...
const benefits = [
  { icon: Timer, label: "Recordatorios automáticos" },
  { icon: MapPinned, label: "Pico y placa" },
  { icon: ShieldAlert, label: "Alertas importantes" },
  { icon: Navigation, label: "Navegación tipo Waze" },
  { icon: Wrench, label: "Novedades de tránsito" },
];
        ...
        <>
          <section className="mb-5 grid gap-3 sm:grid-cols-2">
            {benefits.map((benefit) => {
              ...
              return (
                <div key={benefit.label} className="flex items-center gap-3 rounded-3xl bg-white/90 p-4 shadow-sm ring-1 ring-slate-100">
                  <div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-600 ring-1 ring-blue-100">
                    <Icon size={20} />
                  </div>
                  <p className="font-black text-slate-900">{benefit.label}</p>
                </div>
              );
            })}
          </section>
